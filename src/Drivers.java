...
public class Drivers {
  ...
  public Drivers() {
    ...
    try {
      ...
      while (sc.hasNextLine()) {
        ...
        drivers.add(new Driver(driverArr[0], driverArr[1], Integer.parseInt(driverArr[2])));
  ...
  private void saveToFile() {
    try {
      ...
      Formatter writer = new Formatter("data/drivers.txt");

      this.drivers.forEach((driver) -> {
        writer.format("%s%n", driver.toString());
      });
  ...
  public Driver appoint(String textRestaurantCity) {
    ...
    List < Driver > sortedList = drivers.stream()
      .sorted(Comparator.comparing(Driver::getLoad))
      .filter(d -> d.getCity().equals(textRestaurantCity))
